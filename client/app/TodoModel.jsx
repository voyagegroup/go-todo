...
class TodoModel {
    ...
    addTodo(title) {
        ...
        post('/api/todos', this.token, 201, todo)
        .then(data => {
    ...
    toggleAll(checked) {
        post('/api/todos/toggleall', this.token, 200, {checked})
        .then(() => {
    ...
    toggle(todoToToggle) {
        post('/api/todos/toggle', this.token, 200, todoToToggle)
        .then(() => {
            this.todos = this.todos.map(todo => {
    ...
    destroy(todo) {
        deleteFetch('/api/todos', this.token, 200, todo)
        .then(() => {
    ...
    save(todoToSave, text) {
        ...
        put('/api/todos', this.token, 200, toSave)
        .then(() => {
            this.todos = this.todos.map(todo => {
    ...
    clearCompleted() {
        ...
        deleteFetch('/api/todos/multi', this.token, 200, todosToDelete)
        .then(() => {
